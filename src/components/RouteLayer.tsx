...
function AnimatedFlowDot({ route, animDelay = 0 }: { route: Route; animDelay?: number; }) {
  const map = useMap();
  useEffect(() => {
    if (!map) return;
    const points = route.waypoints.map((wp) => map.latLngToLayerPoint(L.latLng(wp[0], wp[1])));
    ...
    const buildPath = (pts: { x: number; y: number }[]) => pts.map((p, i) => (i === 0 ? 'M ' + p.x + ' ' + p.y : 'L ' + p.x + ' ' + p.y)).join(' ');
    ...
    if (route.type === 'maritime') svg.appendChild(makeAnimDot(3, 0.5, (animDelay + dur / 2) + 's'));
    const pane = map.getPane('overlayPane');
    if (pane) pane.appendChild(svg);
    const updatePos = () => { const newPts = route.waypoints.map((wp) => map.latLngToLayerPoint(L.latLng(wp[0], wp[1]))); pathEl.setAttribute('d', buildPath(newPts)); };
    map.on('zoom move zoomend moveend', updatePos);
    return () => { map.off('zoom move zoomend moveend', updatePos); if (svg.parentNode) svg.parentNode.removeChild(svg); };
  }, [map, route, animDelay]);
  return null;
}
  ...
  return (
    <>
      {visibleRoutes.map((route) => (
        <Polyline key={route.id} positions={route.waypoints} pathOptions={{ color: route.color, weight: route.type === 'maritime' ? 3 : 3.5, opacity: 0.75, dashArray: STATUS_DASH[route.status] || undefined, lineJoin: 'round', lineCap: 'round' }}>
          <Tooltip sticky>
            <div className="route-tooltip">
              <strong>{route.name}</strong><br />
              <span style={{ textTransform: 'capitalize' }}>{route.type}</span>
              {' · '}{route.totalLength.toLocaleString()} {route.lengthUnit}<br />
              <span style={{ color: route.status === 'operational' ? '#16a34a' : route.status === 'under_construction' ? '#d97706' : '#64748b', textTransform: 'capitalize' }}>
      ...
      {visibleRoutes.filter((r) => r.status === 'operational').map((route, i) => (
